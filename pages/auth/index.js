...
function authPage() {
  ...
  const onSubmit = (data) => {
    if (isSignUp) {
      if (data.password !== data.confirmPassword) {
        toast.error(t('form.password_mismatch') || 'Passwords do not match')
        return
      }
      signup(data)
    } else {
  ...
  return (
    <>
      <Head>
        <title>{isSignUp ? 'Sign Up' : 'Login'} - ChainTree</title>
      </Head>
      <div>
          ...
          <div className="m:px-6 w-full rounded px-6 py-6 shadow-lg sm:py-10 md:w-1/2 lg:w-5/12 lg:px-10 xl:w-1/3">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              {isSignUp && (
                <div className="pt-6">
                  ...
                  <input
                    id="name"
                    type="text"
                    className="mt-2 w-full rounded border bg-gray-200 py-3 pl-3 text-xs font-medium leading-none text-gray-800 placeholder-gray-500"
                    placeholder="ex: Satoshi Nakamoto"
                    {...register('name', {
                      required: isSignUp ? (t('form.enter_name') || 'Please enter your name') : false,
                    })}
                  />
                  {errors.name && (
                ...
                <div className="relative flex items-center justify-center">
                  <input
                    ...
                    {...register('password', {
                      ...
                      minLength: {
                        value: 6,
                        message: t('form.verify_password') || 'Minimum 6 characters',
                      },
                    })}
                  />
                  <div
                  ...
                  <div className="relative flex items-center justify-center">
                    <input
                      ...
                      {...register('confirmPassword', {
                        ...
                        validate: (value) =>
                          value === password || (t('form.password_mismatch') || 'Passwords do not match'),
                      })}
                    />
                    <div
                    ...
                    >
                      <img
                        src="/assets/img/eye.svg"
                        alt={showConfirmPass ? 'Hide Password' : 'Show Password'}
                        className="h-4 w-4"
                      />
                    </div>
                  </div>
              ...
              <div className="mt-8">
                <button
                  role="button"
                  className="w-full rounded bg-primary-300 py-4 text-sm font-bold leading-none text-black-400 transition duration-150 ease-in-out hover:bg-primary-400 focus:outline-none focus:ring-0 focus:ring-primary-300 focus:ring-offset-2"
                  type="submit"
                >
