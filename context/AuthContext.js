...
export function AuthProvider({ children }) {
  ...
  const setSession = (session) => {
    if (session) {
      cookie.set('chaintree-bootcamp-auth', session, {
        expires: 1,
      })
      setUserAuthenticated(true)
      setLoading(false)
    } else {
      cookie.remove('chaintree-bootcamp-auth')
  ...
  const signup = (data) => {
    ...
    createUserWithEmailAndPassword(auth, data.email, data.password)
      .then(async (userCredential) => {
        ...
        const userObj = {
          ...userCredential.user,
          displayName: data.name || userCredential.user.displayName,
        }
        setUser(userObj)
        await createUserinFirestore(userObj)
        Router.push('/courses')
      ...
      .catch((error) => {
        if (error.code && error.code.includes('already-in-use')) {
