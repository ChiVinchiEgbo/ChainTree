...
export default function WalletCard() {
  ...
  const handleConnectWallet = async () => {
    ...
    if (publicKey && signMessage) {
      try {
        const message = new TextEncoder().encode('Sign In Message - Logging in ChainTree')
        ...
        console.error('Solana wallet signature error:', error)
        toast.error('Signature failed or rejected', toastParameters)
      }
    } else if (publicKey) {
      const address = publicKey.toBase58()
      if (address && auth.currentUser) await updateUserWalletInFirestore(address, auth.currentUser?.uid)
  ...
  useEffect(() => {
    if (connected && publicKey && auth.currentUser && !userAddress) {
      handleConnectWallet()
    }
  }, [connected, publicKey])
